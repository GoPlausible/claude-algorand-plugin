...
Seven skills cover all three capabilities. Each skill has a `SKILL.md` router plus a `references/` folder with implementation guides and API references.
...
| Error | Cause | Solution |
|-------|-------|----------|
| `Payment transaction signature does not match sender` | Base64 blob corrupted during manual copy-paste into shell/JSON | **NEVER inline base64 blobs manually** — store in shell variables first, interpolate via `${VAR}` (see below) |
...
> **CRITICAL — x402 Base64 Blob Handling**: When constructing `PAYMENT-SIGNATURE` headers via shell/curl, NEVER manually copy-paste base64 blob strings inline into JSON. Base64 blobs from `encode_unsigned_transaction` (bytes) and `wallet_sign_transaction` (blob) are long and even a single character corruption silently breaks signature verification. ALWAYS: (1) store each blob in a separate shell variable, (2) interpolate via `${VAR}`, (3) use `printf '%s'` not `echo`, (4) strip newlines with `tr -d '\n'` (macOS `base64` adds them).

### X402 Skills

Three skills cover x402. Two are for development (building x402 apps), one is for runtime (Claude paying for x402-protected resources directly).

| Task                        | Skill                      |
| --------------------------- | -------------------------- |
| TypeScript x402 development | `algorand-x402-typescript` |
| Python x402 development     | `algorand-x402-python`     |
| Runtime x402 payment (Claude as client) | `algorand-x402-payment` |
